...
import 'package:aspirevue/view/base/alert_dialogs/custom_alert_rols_and_responsibility.dart';
...
class MenteesView extends StatefulWidget {
  const MenteesView({super.key});

  @override
  State<MenteesView> createState() => _MenteesViewState();
}

class _MenteesViewState extends State<MenteesView> {
  @override
  void initState() {
    super.initState();
  }
  ...
  final List _listOfBox = [
    ...
    {"image": AppImages.calendarIc, "text": AppString.roleResponsibilities},
  ];

  @override
  Widget build(BuildContext context) {
    return GetBuilder<MenteesController>(builder: (controller) {
  ...
  Column _buildListTile(MyMentorsData mentorData) {
    return Column(
      children: [
        Container(
          ...
          child: Column(
            children: [
              _buildUserTile(mentorData),
              _buildBoxesRow(mentorData),
            ],
          ),
        ),
        10.sp.sbh,
  ...
  Widget _buildListTileForBox(int index, MyMentorsData data) {
    if (_listOfBox[index]['text'] == "Coaching" && data.agreement == "") {
    ...
    return InkWell(
      onTap: () {
        if (_listOfBox[index]['text'] == "Coaching") {
          ...
          Get.to(
            () => MyCoachDetailsScreen(
              title: "Mentee Detail -  ${data.name.toString()}",
              currentIndex: 0,
              id: data.id.toString(),
              type: "3",
            ),
        ...
        if (_listOfBox[index]['text'] == "Role Responsibilities") {
          // open popup

          showDialog(
            context: context,
            builder: (BuildContext context) {
              return RolesAndResponsibilityAlertDialog(
          ...
          child: Column(
            children: [
              ...
              CustomText(
                ...
                maxLine: 2,
                fontFamily: AppString.manropeFontFamily,
                fontSize: _listOfBox[index]['text'] == "Role Responsibilities"
                    ? 8.sp
                    : 10.sp,
                fontWeight: FontWeight.w600,
              ),
              _listOfBox[index]['text'] == "Role Responsibilities"
                  ? 0.sbh
                  : 10.sp.sbh,
            ],
          ),
