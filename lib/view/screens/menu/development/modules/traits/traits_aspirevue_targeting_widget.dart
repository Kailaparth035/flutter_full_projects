...
class TraitsAspireVueTargetingWidget extends StatefulWidget {
  const TraitsAspireVueTargetingWidget({
    super.key,
  ...
  final String userId;
  @override
  State<TraitsAspireVueTargetingWidget> createState() =>
      _TraitsAspireVueTargetingState();
}

class _TraitsAspireVueTargetingState
    extends State<TraitsAspireVueTargetingWidget> {
  final _traitsController = Get.find<TraitsController>();

  @override
  void initState() {
    super.initState();
    _traitsController.getTargetingDetails(widget.userId);
  }
  ...
  _callAPI(DevelopmentController controller, SliderData data,
    ...
    controller.updateSelfReflaction(
      ...
      newScore: newScore,
      onReaload: (val) async {},
      ratingType: _traitsController.dataTarget!.ratingType.toString(),
      userId: _traitsController.dataTarget!.userId.toString(),
      type: "",
    );
  }
  ...
  Widget _buildMainView() {
    return GetBuilder<TraitsController>(builder: (traitsController) {
      if (traitsController.isLoadingTarget) {
        return const Center(child: CustomLoadingWidget());
      }
      if (traitsController.isErrorTarget ||
          traitsController.dataTarget == null) {
        return Center(
          child: CustomErrorWidget(
            ...
            text: traitsController.isErrorTarget
                ? traitsController.errorMsgTarget
        ...
        return Stack(
          children: [
            _buildView(traitsController.dataTarget!,
                traitsController.titleListTarget, traitsController),
            traitsController.dataTarget!.isJourneyLicensePurchased == 0
                ? PurchasePopupWidget(
                    text: traitsController
  ...
  Widget _buildView(TraitAssesData dataTarget, List titleListTarget,
    ...
    return CustomSlideUpAndFadeWidget(
      child: RefreshIndicator(
        onRefresh: () {
          return _traitsController.getTargetingDetails(widget.userId);
        },
        child: SingleChildScrollView(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                buildBlackTitle(
                    "Personality Profiler: Targeting My Ideal Self at My Best"),
                5.sp.sbh,
                buildGreyTitle(
                    ...
                    : Column(
                        children: [
                          _buildButtonwithIcon(
                              traitsController.isSharedTargating),
                          15.sp.sbh,
                        ],
                      ),
  ...
  shareWithSupervisor(bool isChecked) async {
    bool? result = await Get.find<DevelopmentController>().shareWithSupervisor(
        styleId: _traitsController.dataTarget!.styleId.toString(),
        userId: _traitsController.dataTarget!.userId.toString(),
    ...
    if (result != null && result == true) {
      ...
      _traitsController.updateShareStatus();
    }
  }
}
