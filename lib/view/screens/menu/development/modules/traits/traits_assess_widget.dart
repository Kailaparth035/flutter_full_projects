...
class TraitsAssessWidget extends StatefulWidget {
  const TraitsAssessWidget({
  ...
  @override
  State<TraitsAssessWidget> createState() => _TraitsAssesstState();
}

class _TraitsAssesstState extends State<TraitsAssessWidget> {
  final _traitsController = Get.find<TraitsController>();

  @override
  void initState() {
    super.initState();
    _traitsController.getAssessData(widget.userId);
  }
  ...
  Widget _buildMainView() {
    return GetBuilder<TraitsController>(builder: (traitsController) {
      if (traitsController.isLoadingAssess) {
        return const Center(child: CustomLoadingWidget());
      }
      if (traitsController.isErrorAssess ||
          traitsController.dataAssess == null) {
        return Center(
          child: CustomErrorWidget(
            width: 40.w,
            onRetry: () {
              traitsController.getAssessData(widget.userId);
            },
            text: traitsController.isErrorAssess
                ? traitsController.errorMsgAssess
                : AppString.somethingWentWrong,
          ),
        );
      } else {
        return _buildView(traitsController.dataAssess!, traitsController);
      }
    });
  }

  Widget _buildView(
      TraitAssesData dataAssess, TraitsController traitsController) {
    return CustomSlideUpAndFadeWidget(
      child: RefreshIndicator(
        onRefresh: () {
          return _traitsController.getAssessData(widget.userId);
        },
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                buildDivider(),
                buildTitleLog(traitsController.titleList),
                buildDivider(),
                15.sp.sbh,
