...
import 'package:aspirevue/view/screens/menu/development/widgets/development_cognitive_obtactls_widget.dart';
...
class CognitiveAspirevueTrainingWidget extends StatefulWidget {
  const CognitiveAspirevueTrainingWidget({super.key, required this.userId});

  final String userId;
  @override
  State<CognitiveAspirevueTrainingWidget> createState() =>
      _CognitiveAspirevueTrainingtState();
}

class _CognitiveAspirevueTrainingtState
    extends State<CognitiveAspirevueTrainingWidget> {
  final _cognitiveController = Get.find<CognitiveController>();

  @override
  void initState() {
    super.initState();
    _cognitiveController.getTargetingDetails(widget.userId);
  }
  ...
  _callAPI(DevelopmentController controller, SliderData data,
    ...
    controller.updateSelfReflaction(
      ...
      newScore: newScore,
      onReaload: (val) async {},
      ratingType: _cognitiveController.dataTarget!.ratingType.toString(),
      userId: _cognitiveController.dataTarget!.userId.toString(),
      type: "",
    );
  }
  ...
  Widget _buildMainView() {
    return GetBuilder<CognitiveController>(builder: (cognitiveController) {
      if (cognitiveController.isLoadingTarget) {
        return const Center(child: CustomLoadingWidget());
      }
      if (cognitiveController.isErrorTarget ||
          cognitiveController.dataTarget == null) {
        return Center(
          child: CustomErrorWidget(
            ...
            text: cognitiveController.isErrorTarget
                ? cognitiveController.errorMsgTarget
        ...
        return Stack(
          children: [
            _buildView(cognitiveController.dataTarget!,
                cognitiveController.titleListTarget, cognitiveController),
            cognitiveController.dataTarget!.isJourneyLicensePurchased == 0
                ? PurchasePopupWidget(
  ...
  Widget _buildView(TraitAssesData dataTarget, List titleListTarget,
    ...
    return CustomSlideUpAndFadeWidget(
      child: RefreshIndicator(
        onRefresh: () {
          return _cognitiveController.getTargetingDetails(widget.userId);
        },
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                10.sp.sbh,
                buildBlackTitle(
                    "Cognitive Profiler: Targeting My Ideal Self at My Best"),
                5.sp.sbh,
                buildGreyTitle(
                    "For each scale below, consider the various data sources and move the MyTarget slider to your ideal."),
                10.sp.sbh,
                buildDivider(),
                ...
                dataTarget.relationWithLoginUser ==
                    ...
                    : Column(
                        children: [
                          _buildSharedButton(
                              cognitiveController.isSharedTargating),
                          15.sp.sbh,
                        ],
                      ),
                ...
                10.sp.sbh,
                buildTitleWithBG("Cognitive Obstacles"),
                Padding(
                  padding:
                      EdgeInsets.symmetric(vertical: 5.sp, horizontal: 7.sp),
                  child: buildGreyTitle(
                      "Our mind can sometimes convince us of something that isn't really true. Unhelpful thought patterns can carry over from past experiences, reinforce negative thinking or emotions, and influence us with ideas that sound extremely simple, rational, and accurate - but wrong. Insight and awareness can help overcome obstacles to clear thinking."),
                ),
                10.sp.sbh,
                ...dataTarget.cognitiveObstacles!
                    .map((e) => DevelopmentCognitiveObtaclsWidget(data: e))
              ],
            ),
          ),
        ),
  ...
  Center _buildSharedButton(bool isShared) {
    return Center(
      child: CustomButton2(
  ...
  shareWithSupervisor(bool isShared) async {
    bool? result = await Get.find<DevelopmentController>().shareWithSupervisor(
        styleId: _cognitiveController.dataTarget!.styleId.toString(),
        userId: _cognitiveController.dataTarget!.userId.toString(),
        assessId: isShared == true ? "0" : "1");

    if (result != null && result == true) {
      _cognitiveController.updateShareStatus();
    }
  }
}
