...
class ValuesAssessWidget extends StatefulWidget {
  const ValuesAssessWidget({super.key, required this.userId});

  final String userId;

  @override
  State<ValuesAssessWidget> createState() => _ValuesAssesstState();
}

class _ValuesAssesstState extends State<ValuesAssessWidget> {
  final _valuesController = Get.find<ValuesController>();
  @override
  void initState() {
    super.initState();
    _valuesController.getAssessData(widget.userId);
  }
  ...
  Widget _buildMainView() {
    return GetBuilder<ValuesController>(builder: (valuesController) {
      if (valuesController.isLoadingAssess) {
        return const Center(child: CustomLoadingWidget());
      }
      if (valuesController.isErrorAssess ||
          valuesController.dataAssess == null) {
        return Center(
          child: CustomErrorWidget(
            width: 40.w,
            onRetry: () {
              valuesController.getAssessData(widget.userId);
            },
            text: valuesController.isErrorAssess
                ? valuesController.errorMsgAssess
                : AppString.somethingWentWrong,
          ),
        );
      } else {
        return _buildView(
            valuesController.dataAssess!, valuesController.titleListAssess);
      }
    });
  }
  ...
  Widget _buildView(TraitAssesData dataAssess, List titleListAssess) {
    return CustomSlideUpAndFadeWidget(
      child: RefreshIndicator(
        onRefresh: () {
          return _valuesController.getAssessData(widget.userId);
        },
